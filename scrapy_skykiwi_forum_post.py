...
def connect_db_read_post():
	...
	conn.commit()
	try:
		cur = cursor.execute("SELECT * FROM skykiwi_forum_room_rent WHERE published = No")
		# 获取数据库中表的全部数据
		data = cur.fetchall()
		if data:
			d = open_Driver()
			for row in data:
				...
				post_message = row[3]
				try:
					post(d, url, title, sub_title, post_message)
					try:
						cursor.execute("UPDATE skykiwi_forum_room_rent SET published = Yes WHERE url = '%s'" % url)
						conn.commit()
...
def post(d, url, title, sub_title, post_message):
	directory = r"D:\Desktop\img_skykiwi_forum\租房中心" + '\\' + url.split('=')[-2][:-6]
	c = 1
	cc = 1
	...
	if d.find_elements_by_xpath('//*[contains(@id, "attach_list_")]'):
...
def open_Driver():
	...
	d = webdriver.Chrome()
	d.get('http://tangren.co.nz/forum.php?mod=post&action=newthread&fid=42')
	d.maximize_window()
	time.sleep(5)
...
def test():
	...
	d.find_element_by_xpath('//*[@id="e_iframe"]').send_keys(title)
	time.sleep(3)
