...
def get_info(host, url):
	...
	title = soup.find('h1').findAll('a')[0].text.strip() + soup.find('h1').findAll('a')[1].text.strip()
	if host.split('=')[-1] == '19':
		local_path += '\\' + '租房中心'
	...
	if soup.find('td', {'class':'t_f'}).findAll('i'):
		...
		post_message = post_message.split(i_node)[1]
...
def get_img(imgs, url, local_path):
	...
	if imgs:
		for img in imgs:
			if ('bbs.skykiwi.com' in img.get('src')) and ((img_src[-3:] == 'gif') or (img_src[-3:] == 'png') or (img_src[-4:] == 'jpeg') or (img_src[-3:] == 'jpg')):
...
def save_info_to_db(info_list):
	...
	# 创建表格
	try:
		cursor.execute("CREATE TABLE skykiwi_forum_room_rent(url Text, title Text, sub_title Text, post_message Memo, published Boolean)")
		conn.commit()
	except Exception as ex:
		print("出现如下异常: %s" % ex)
		print("创建表格失败")
	...
	for info in info_list:
		...
		result = cursor.execute("SELECT * FROM skykiwi_forum_room_rent WHERE url = '%s'" % url)
		data = result.fetchall()
		try:
			if not data:
				sql = "INSERT INTO skykiwi_forum_room_rent VALUES('%s', '%s', '%s', '%s')" % (url, title, sub_title, post_message)
				cursor.execute(sql)
				conn.commit()
		except Exception as ex:
			print("出现如下异常: %s" % ex)
...
def main():
	'''main function'''
	info_list = []
	host = 'http://bbs.skykiwi.com/forum.php?mod=forumdisplay&fid=19'
